...
class ProjectsReorganizer:
    ...
    def __init__(self, projects_root: Path, dry_run: bool = True):
        ...
        self.stats = {"dirs_moved": 0, "errors": 0}
    ...
    def move_directory(self, source: Path, target: Path):
        ...
        try:
            if target.exists():
                print_error(
                    f"Target already exists: {target.relative_to(self.projects_root)}"
                )
    ...
    def print_summary(self):
        ...
        print(
            f"Directories moved:  {Colors.OKGREEN}{self.stats['dirs_moved']}{Colors.ENDC}"
        )
        ...
        if self.dry_run:
            ...
            print(
                f"{Colors.OKCYAN}Run with --execute flag to perform actual reorganization{Colors.ENDC}"
            )
        else:
            print(
