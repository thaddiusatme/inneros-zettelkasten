...
class ProjectsCleanup:
    ...
    def ensure_target_dir(self, target_dir: Path):
        ...
        if not target_dir.exists():
            if self.dry_run:
                print_info(
                    f"Would create directory: {target_dir.relative_to(self.projects_root)}"
                )
            else:
                target_dir.mkdir(parents=True, exist_ok=True)
                print_success(
                    f"Created directory: {target_dir.relative_to(self.projects_root)}"
                )
    ...
    def move_file(self, source: Path, target: Path):
        ...
        try:
            if target.exists():
                print_warning(
                    f"Conflict: {target.name} already exists in {target.parent.name}/"
                )
    ...
    def move_directory(self, source: Path, target: Path):
        ...
        try:
            if target.exists():
                print_warning(
                    f"Target directory exists: {target.relative_to(self.projects_root)}"
                )
                print_info(
                    f"Merging contents from: {source.relative_to(self.projects_root)}"
                )
                ...
                if self.dry_run:
                    ...
                    print_info(
                        f"  From: {source.parent.relative_to(self.projects_root)}/"
                    )
                    print_info(
                        f"  To:   {target.parent.relative_to(self.projects_root)}/"
                    )
                else:
                    self.ensure_target_dir(target.parent)
                    shutil.move(str(source), str(target))
                    print_success(
                        f"Moved directory: {source.name}/ → {target.parent.name}/"
                    )
    ...
    def remove_empty_dir(self, dir_path: Path):
        ...
        try:
            ...
            if any(dir_path.iterdir()):
                print_warning(
                    f"Directory not empty, skipping: {dir_path.relative_to(self.projects_root)}"
                )
                return

            if self.dry_run:
                print_info(
                    f"Would remove empty directory: {dir_path.relative_to(self.projects_root)}"
                )
            else:
                dir_path.rmdir()
                print_success(
                    f"Removed empty directory: {dir_path.relative_to(self.projects_root)}"
                )
    ...
    def phase2_reorganize_standalone_files(self):
        ...
        mappings = {
            ...
            "executive-report-stakeholders-draft-3.md": self.projects_root
            / "REFERENCE",
            "inneros-gamification-discovery-manifest.md": self.projects_root
            / "REFERENCE",
            "logging-monitoring-requirements-automation.md": self.projects_root
            / "REFERENCE",
            # To COMPLETED-2025-09/
            "automation-completion-retrofit-manifest.md": self.projects_root
            / "COMPLETED-2025-09",
            # To COMPLETED-2025-10/
            "workflow-demo-deprecation-plan.md": self.projects_root
            / "COMPLETED-2025-10",
            "workflow-demo-extraction-status.md": self.projects_root
            / "COMPLETED-2025-10",
            # To DEPRECATED/
            "youtube-official-api-integration-manifest-deprecated-2025-10-09.md": self.projects_root
            / "DEPRECATED",
        }
    ...
    def print_summary(self):
        ...
        print(
            f"Files moved:        {Colors.OKGREEN}{self.stats['files_moved']}{Colors.ENDC}"
        )
        print(
            f"Directories created: {Colors.OKCYAN}{self.stats['dirs_created']}{Colors.ENDC}"
        )
        print(
            f"Directories removed: {Colors.WARNING}{self.stats['dirs_removed']}{Colors.ENDC}"
        )

        if self.stats["errors"] > 0:
            print(
                f"Errors encountered:  {Colors.FAIL}{self.stats['errors']}{Colors.ENDC}"
            )
        else:
        ...
        if self.dry_run:
            ...
            print(
                f"{Colors.OKCYAN}Run with --execute flag to perform actual cleanup{Colors.ENDC}"
            )
