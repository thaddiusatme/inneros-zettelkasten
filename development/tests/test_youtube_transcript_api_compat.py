    ...
    def test_fetch_returns_correct_structure(self):
        ...
        try:
            ...
            for transcript in transcript_list:
                ...
                assert hasattr(
                    fetched, "snippets"
                ), "FetchedTranscript must have 'snippets' attribute"
                assert hasattr(
                    fetched, "is_generated"
                ), "FetchedTranscript must have 'is_generated' attribute"
                assert hasattr(
                    fetched, "language_code"
                ), "FetchedTranscript must have 'language_code' attribute"
                assert hasattr(
                    fetched, "video_id"
                ), "FetchedTranscript must have 'video_id' attribute"
                ...
                if len(fetched.snippets) > 0:
                    ...
                    assert hasattr(
                        snippet, "text"
                    ), "Snippet must have 'text' attribute"
                    assert hasattr(
                        snippet, "start"
                    ), "Snippet must have 'start' attribute"
                    assert hasattr(
                        snippet, "duration"
                    ), "Snippet must have 'duration' attribute"
    ...
    def test_transcript_list_structure(self):
        ...
        try:
            ...
            for transcript in transcripts:
                ...
                ), "Transcript must have 'language_code'"
                assert hasattr(
                    transcript, "is_generated"
                ), "Transcript must have 'is_generated'"
                assert hasattr(
                    transcript, "fetch"
                ), "Transcript must have 'fetch' method"
