...
class TestZettelkastenIntegration(unittest.TestCase):
    ...
    def test_ai_workflow_integration_exists(self):
        ...
        self.assertTrue(
            callable(getattr(self.matcher, "process_capture_notes_with_ai"))
        )
    ...
    def test_ai_workflow_integration_returns_correct_structure(self):
        ...
        capture_note = self.matcher.generate_capture_note(
            test_pair, "test-ai-integration"
        )
    ...
    def test_ai_quality_scoring_zettelkasten_optimization(self):
        ...
        capture_note = self.matcher.generate_capture_note(
            zettelkasten_pair, "atomic-concept-test"
        )
        result = self.matcher.process_capture_notes_with_ai([capture_note])
    ...
    def test_zettelkasten_tag_generation(self):
        ...
        capture_note = self.matcher.generate_capture_note(
            test_pair, "zettelkasten-tagging-test"
        )
        result = self.matcher.process_capture_notes_with_ai([capture_note])
        ...
        if result["ai_results"]:
            ...
            self.assertTrue(
                has_expected, f"Expected Zettelkasten concepts not in tags: {tags}"
            )
    ...
    def test_capture_note_contains_clickable_links(self):
        ...
        capture_note = self.matcher.generate_capture_note(
            test_pair, "clickable-links-test"
        )
    ...
    def test_zettelkasten_yaml_frontmatter_structure(self):
        ...
        capture_note = self.matcher.generate_capture_note(
            test_pair, "zettelkasten-yaml-test"
        )
        content = capture_note["markdown_content"]
    ...
    def test_atomic_concept_ready_detection(self):
        ...
        if result["ai_results"]:
            ...
            has_development = any(
                indicator in rec_text for indicator in development_indicators
            )
            self.assertTrue(
                has_development,
                f"No development indicators in recommendations: {recommendations}",
            )
    ...
    def test_connection_opportunity_detection(self):
        ...
        capture_note = self.matcher.generate_capture_note(
            connection_pair, "connection-test"
        )
        result = self.matcher.process_capture_notes_with_ai([capture_note])
        ...
        if result["ai_results"]:
            ...
            has_knowledge_concepts = any(
                any(concept in tag.lower() for concept in tag_concepts) for tag in tags
            )
            self.assertTrue(
                has_knowledge_concepts, f"No knowledge concepts in tags: {tags}"
            )
    ...
    def test_batch_processing_performance(self):
        ...
        self.assertLess(
            processing_time,
            30.0,
            f"Batch processing took {processing_time}s, exceeds 30s target",
        )
