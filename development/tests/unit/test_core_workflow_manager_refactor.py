...
class TestCoreWorkflowManagerOrchestration:
    ...
    def test_core_orchestrates_all_managers_with_exception_handling(
        ...
        mock_analytics_manager.assess_quality.assert_called_once_with(
            "test.md", dry_run=True
        )
        mock_ai_enhancement_manager.enhance_note.assert_called_once_with(
            "test.md", fast=False, dry_run=True
        )
        mock_connection_manager.discover_links.assert_called_once_with(
            "test.md", dry_run=True
        )
...
class TestCoreExceptionHandling:
    ...
    def test_core_stops_on_analytics_validation_error(
        ...
        # Arrange
        mock_analytics_manager.assess_quality.side_effect = ValueError(
    ...
    def test_core_stops_on_analytics_file_not_found_error(
        ...
        # Arrange
        mock_analytics_manager.assess_quality.side_effect = FileNotFoundError(
            "Note file not found"
        )
    ...
    def test_core_continues_on_ai_failure_with_degraded_result(
        ...
        mock_ai_enhancement_manager.enhance_note.side_effect = Exception(
...
class TestCoreAICostGating:
    ...
    def test_core_skips_ai_when_quality_score_below_threshold(
        ...
        assert any(
            "AI" in str(w) and "skipped" in str(w) for w in result.get("warnings", [])
        )
...
class TestCoreBugReporting:
    ...
    def test_core_creates_bug_report_on_total_failure(
        ...
        # Arrange
        mock_analytics_manager.assess_quality.side_effect = Exception(
            "Analytics critical error"
        )
        mock_ai_enhancement_manager.enhance_note.side_effect = Exception(
            "AI total failure"
        )
        mock_connection_manager.discover_links.side_effect = Exception(
            "Connections failed"
        )
        ...
        assert any(
            "Total workflow failure" in str(w) for w in result.get("warnings", [])
        )

        # Assert - Bug report created
        bug_reports = list(
            (mock_base_dir / ".automation" / "review_queue").glob(
...
class TestCoreDryRunMode:
    ...
    def test_dry_run_prevents_save_operations(
        ...
        mock_analytics_manager.assess_quality.assert_called_with(
            "test.md", dry_run=True
        )
        mock_ai_enhancement_manager.enhance_note.assert_called_with(
            "test.md", fast=False, dry_run=True
        )
        mock_connection_manager.discover_links.assert_called_with(
            "test.md", dry_run=True
        )

        # Assert - Result indicates dry_run
        # (Implementation detail: Core should track dry_run in result)
        assert result.get("dry_run") == True or "dry_run" in str(
            result.get("warnings", [])
        )
