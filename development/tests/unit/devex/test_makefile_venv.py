...
class TestMakefileVenvConfiguration:
    ...
    def test_defines_venv_variable(self):
        ...
        assert re.search(
            r"^VENV\s*[:?]?=\s*\.venv", content, re.MULTILINE
    ...
    def test_defines_python_variable_using_venv(self):
        ...
        assert re.search(
            r"^PYTHON\s*[:?]?=\s*\$\(VENV\)/bin/python", content, re.MULTILINE
    ...
    def test_lint_target_uses_venv_python(self):
        ...
        lint_match = re.search(
            r"^lint:.*?(?=^[a-z]|\Z)", content, re.MULTILINE | re.DOTALL
        )
    ...
    def test_type_target_uses_venv_python(self):
        ...
        type_match = re.search(
            r"^type:.*?(?=^[a-z]|\Z)", content, re.MULTILINE | re.DOTALL
        )
        ...
        assert (
            "python3 -m pyright" not in type_block
    ...
    def test_venv_bootstrap_target_exists(self):
        ...
        has_venv_target = (
            re.search(r"^\$\(VENV\)/bin/activate:", content, re.MULTILINE)
            or re.search(r"^\.venv/bin/activate:", content, re.MULTILINE)
            or re.search(r"^venv:", content, re.MULTILINE)
        )
        assert (
            has_venv_target
    ...
    def test_lint_depends_on_venv(self):
        ...
        has_venv_dep = (
            "$(VENV)/bin/activate" in deps or "venv" in deps or ".venv" in deps
        )
        assert (
            has_venv_dep
...
class TestDevRequirementsFile:
    ...
    def test_dev_requirements_exists(self):
        ...
        assert (
            self.DEV_REQUIREMENTS_PATH.exists()
