...
class TestAutoPromoteSubcommandRegistration:
    ...
    def test_auto_promote_has_quality_threshold_arg(self):
        ...
        args = parser.parse_args(['test-vault', 'auto-promote', '--quality-threshold', '0.8'])
        assert hasattr(args, 'quality_threshold'), "--quality-threshold arg not registered"
        assert args.quality_threshold == 0.8, "--quality-threshold value not parsed correctly"

        # Test default value
        args_default = parser.parse_args(['test-vault', 'auto-promote'])
        assert args_default.quality_threshold == 0.7, "--quality-threshold default should be 0.7"
    ...
    def test_auto_promote_has_format_arg(self):
        ...
        # Test normal format
        args_normal = parser.parse_args(['test-vault', 'auto-promote', '--format', 'normal'])
        assert hasattr(args_normal, 'format'), "--format arg not registered"
        assert args_normal.format == 'normal', "--format normal not parsed"

        # Test json format
        args_json = parser.parse_args(['test-vault', 'auto-promote', '--format', 'json'])
...
class TestMainFunctionAutoPromoteExecution:
    ...
    def test_main_passes_quality_threshold(self, mock_cli_class):
        ...
        test_args = ['core_workflow_cli.py', 'test-vault', 'auto-promote', '--quality-threshold', '0.8']
        ...
        call_kwargs = mock_cli_instance.auto_promote.call_args[1]
        assert call_kwargs.get('quality_threshold') == 0.8, "quality_threshold not passed"
    ...
    def test_main_passes_format_arg(self, mock_cli_class):
        ...
        test_args = ['core_workflow_cli.py', 'test-vault', 'auto-promote', '--format', 'json']
        ...
        call_kwargs = mock_cli_instance.auto_promote.call_args[1]
        assert call_kwargs.get('output_format') == 'json', "output_format parameter not passed"
    ...
    def test_main_passes_all_arguments_combined(self, mock_cli_class):
        ...
        test_args = [
            'core_workflow_cli.py', 'test-vault', 'auto-promote',
            '--dry-run',
        ...
        assert call_kwargs.get('quality_threshold') == 0.85, "quality_threshold not passed"
...
class TestAutoPromoteHelpText:
    ...
    def test_quality_threshold_help_explains_range(self):
        ...
        assert 'auto-promote --quality-threshold' in help_text, "Quality threshold example not in help"
    ...
    def test_dry_run_help_explains_preview(self):
        ...
        assert 'Preview' in help_text or 'preview' in help_text, "Preview concept not in help"
