...
"""
        ...
        with patch("src.ai.youtube_transcript_saver.YouTubeTranscriptSaver"), patch(
            "pathlib.Path.read_text", return_value=note_content
        ...
        assert (
            first_call_metadata.get("status") == "processing"
        ), "First update should change status to 'processing'"
        assert (
            "processing_started_at" in first_call_metadata
...
"""
        ...
        assert (
            len(calls) >= 2
        ), "Should call update_frontmatter at least twice (start + end)"

        # Last call should set status to processed
        last_call_metadata = (
            calls[-1][0][1] if calls[-1][0] else calls[-1][1]["metadata"]
        )
        assert (
            last_call_metadata.get("status") == "processed"
        ), "Final update should change status to 'processed'"
        assert (
            "processing_completed_at" in last_call_metadata
        ), "Should add processing_completed_at timestamp"
        assert (
            last_call_metadata.get("ai_processed") is True
...
Notes"""
        ...
        for call_args in calls:
            ...
            if "ready_for_processing" in metadata:
                assert (
                    metadata["ready_for_processing"] is True
...
Notes"""
        ...
        assert (
            "processing_started_at" in first_metadata
        ...
        assert (
            "processing_completed_at" in last_metadata
...
Notes"""
        ...
        with patch("src.ai.youtube_transcript_saver.YouTubeTranscriptSaver"), patch(
            ...
            MockFetcher.return_value.fetch_transcript.side_effect = Exception(
                "API Error"
            )
...
Notes"""
        ...
        assert (
            last_metadata.get("ai_processed") is True
        ), "Should maintain backward compatibility with ai_processed flag"
        assert (
            last_metadata.get("status") == "processed"
