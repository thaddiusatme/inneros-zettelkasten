...
class TestProcessingTimeMetrics:
    ...
    def test_smart_link_handler_tracks_processing_time(self):
        ...
        with patch.object(
            handler.link_integrator,
            "process_note_for_links",
            return_value={"success": True, "suggestions_count": 3},
        ):
            handler.process(test_file, "created")
...
class TestPerformanceThresholds:
    ...
    def test_screenshot_handler_warns_when_exceeding_threshold(self, caplog):
        ...
        with patch.object(
            ...
            with patch(
                "time.time", side_effect=[0, 10.0]
            ):  # Start=0, End=10 (exceeds 5s threshold)
                handler.process(test_file, "created")
    ...
    def test_smart_link_handler_warns_when_exceeding_threshold(self, caplog):
        ...
        with patch.object(
            ...
            with patch(
                "time.time", side_effect=[0, 8.0]
            ):  # Start=0, End=8 (exceeds 5s threshold)
                handler.process(test_file, "created")
