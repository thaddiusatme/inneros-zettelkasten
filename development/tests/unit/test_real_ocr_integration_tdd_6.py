...
class TestRealOCRIntegration(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.screenshot_path = (
            Path(self.temp_dir) / "Screenshot_20250925_092059_Messenger.jpg"
        )
    ...
    def test_real_llama_vision_ocr_integration(self):
        ...
        with patch.object(LlamaVisionOCR, "analyze_screenshot") as mock_analyze:
            ...
            result = self.analyzer.analyze_screenshot_with_rich_context(
                self.screenshot_path
            )
            ...
            self.assertIn(
                "Messenger conversation between John and Sarah",
    ...
    def test_detailed_ai_vision_description_requirement(self):
        ...
        with patch.object(LlamaVisionOCR, "analyze_screenshot") as mock_analyze:
            ...
            self.assertGreater(
                len(description), 100, "AI vision description must be >100 words"
            )
            self.assertIn(
                "visual", description.lower(), "Must describe visual elements"
            )
            self.assertIn(
                "interface", description.lower(), "Must describe UI components"
            )
    ...
    def test_source_context_analysis_with_app_detection(self):
        ...
        with patch.object(LlamaVisionOCR, "analyze_screenshot") as mock_analyze:
            ...
            threads_screenshot = (
                Path(self.temp_dir) / "Screenshot_20250925_143021_Threads.jpg"
            )
    ...
    def test_enhanced_filename_generation_with_real_ocr_content(self):
        ...
        with patch.object(LlamaVisionOCR, "analyze_screenshot") as mock_analyze:
            ...
            filename = self.filename_generator.generate_contextual_filename(
                self.screenshot_path, mock_analyze.return_value, timestamp
            )

            # Verify enhanced filename from real content - WILL FAIL with mock keywords
            self.assertEqual(
                filename, "capture-20250925-1430-github-ai-automation-tools.md"
            )
    ...
    def test_app_specific_conversation_analysis(self):
        ...
        with patch.object(LlamaVisionOCR, "analyze_screenshot") as mock_analyze:
            mock_analyze.return_value = VisionAnalysisResult(
                ...
                key_insights=[
                    "Positive sentiment about AI tools",
    ...
    def test_error_handling_and_fallback_strategies(self):
        ...
        with patch.object(LlamaVisionOCR, "analyze_screenshot") as mock_analyze:
            ...
            self.assertIn(
                "Visual content captured but OCR processing failed",
...
class TestOCRUtilityExtraction(unittest.TestCase):
    ...
    def test_content_intelligence_analyzer_utility_class(self):
        ...
        intelligence = analyzer.analyze_app_specific_content(
            "messaging_app", "conversation text"
        )
