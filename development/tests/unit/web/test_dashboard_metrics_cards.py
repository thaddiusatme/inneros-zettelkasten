...
class TestDashboardMetricsCards:
    ...
    def test_dashboard_renders_template(self, client):
        ...
        assert 'dashboard' in html.lower() or 'metrics' in html.lower(), \
    ...
    def test_dashboard_has_metrics_section(self, client):
        ...
        has_metrics_section = (
            'id="metrics-cards"' in html or
            'class="metrics-cards"' in html or
            re.search(r'<section[^>]*class="[^"]*metrics[^"]*"', html, re.IGNORECASE)
        )

        assert has_metrics_section, \
    ...
    def test_metrics_section_has_card_grid(self, client):
        ...
        has_grid = (
            'class="row"' in html or
            'class="grid"' in html or
            'class="metrics-grid"' in html or
            re.search(r'class="[^"]*grid[^"]*"', html, re.IGNORECASE)
        )

        assert has_grid, \
            "Metrics section should use grid layout for responsive cards"
    ...
    def test_counter_card_elements_exist(self, client):
        ...
        has_counter_card = (
            ...
            re.search(r'class="[^"]*counter[^"]*card[^"]*"', html, re.IGNORECASE)
        )
        ...
        has_value_element = (
            'data-metric-value' in html or
            re.search(r'class="[^"]*metric-value[^"]*"', html, re.IGNORECASE)
        )

        assert has_value_element, \
    ...
    def test_gauge_card_elements_exist(self, client):
        ...
        has_gauge_card = (
            'data-metric-type="gauge"' in html or
            re.search(r'class="[^"]*gauge[^"]*card[^"]*"', html, re.IGNORECASE)
        )

        assert has_gauge_card, \
            "Dashboard should have at least one gauge metric card"
    ...
    def test_histogram_card_elements_exist(self, client):
        ...
        has_histogram_card = (
            'data-metric-type="histogram"' in html or
            re.search(r'class="[^"]*histogram[^"]*card[^"]*"', html, re.IGNORECASE)
        )

        assert has_histogram_card, \
    ...
    def test_metrics_cards_have_error_state_elements(self, client):
        ...
        has_error_elements = (
            'data-error-message' in html or
            re.search(r'class="[^"]*error[^"]*"', html, re.IGNORECASE)
        )

        assert has_error_elements, \
    ...
    def test_metrics_cards_have_loading_state_elements(self, client):
        ...
        has_loading_elements = (
            'data-loading' in html or
            re.search(r'class="[^"]*loading[^"]*"', html, re.IGNORECASE) or
            re.search(r'class="[^"]*spinner[^"]*"', html, re.IGNORECASE)
        )

        assert has_loading_elements, \
    ...
    def test_metrics_cards_have_timestamp_display(self, client):
        ...
        has_timestamp = (
            '<time' in html or
            'data-timestamp' in html or
            re.search(r'class="[^"]*timestamp[^"]*"', html, re.IGNORECASE)
        )

        assert has_timestamp, \
            "Dashboard should display timestamp for metrics freshness"
...
class TestMetricsJavaScriptIntegration:
    ...
    def test_metrics_js_has_metricsupdater_class(self):
        ...
        assert 'class MetricsUpdater' in content or 'function MetricsUpdater' in content, \
    ...
    def test_metrics_js_has_fetch_logic(self):
        ...
        assert 'fetch(' in content and '/api/metrics' in content, \
...
class TestDashboardCSS:
    ...
    def test_dashboard_css_has_card_styles(self):
        ...
        assert 'card' in content.lower() or 'metric' in content.lower(), \
