...
class TestDashboardLauncher:
    ...
    def test_launch_handles_missing_dashboard_file(self):
        ...
        with patch('subprocess.Popen', side_effect=FileNotFoundError("Dashboard not found")):
            result = launcher.launch()
...
class TestTerminalDashboardLauncher:
    ...
    def test_launch_live_passes_daemon_url(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert any('9999' in str(arg) for arg in call_args[0] + tuple(call_args[1].values()))
    ...
    def test_launch_live_handles_keyboard_interrupt(self):
        ...
        with patch('subprocess.run', side_effect=KeyboardInterrupt()):
            ...
            assert 'stopped' in result.get('message', '').lower() or 'interrupted' in result.get('message', '').lower()
...
class TestDashboardOrchestrator:
    ...
    def test_default_launches_web_dashboard(self):
        ...
        with patch('subprocess.Popen') as mock_popen:
            ...
            assert result['success'] is True
            assert result.get('mode') == 'web' or 'workflow' in result.get('message', '').lower()
    ...
    def test_live_flag_launches_terminal_dashboard(self):
        ...
        with patch('subprocess.run') as mock_run:
            ...
            assert result['success'] is True
            assert result.get('mode') == 'live' or 'terminal' in result.get('message', '').lower()
