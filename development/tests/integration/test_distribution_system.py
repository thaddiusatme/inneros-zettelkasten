...
class TestDistributionCreation:
    ...
    def temp_source_repo(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (source_dir / "Reviews" / "daily-2025-10-09.md").write_text("Personal review")
            ...
            if (repo_root / "scripts" / "create-distribution.sh").exists():
                shutil.copy(repo_root / "scripts" / "create-distribution.sh", scripts_dir)
...
class TestSecurityAudit:
    ...
    def test_detects_personal_name_patterns(self, temp_audit_dir):
        ...
        assert "thaddius" in result.stdout.lower() or "personal" in result.stdout.lower()
    ...
    def test_detects_username_patterns(self, temp_audit_dir):
        ...
        assert result.returncode != 0
        assert "username" in result.stdout.lower() or "thaddiusatme" in result.stdout.lower()
    ...
    def test_json_output_format(self, temp_audit_dir):
        ...
        result = subprocess.run(
            ["python3", "scripts/security-audit.py", str(temp_audit_dir), "--format=json"],
            capture_output=True,
            text=True
        )
