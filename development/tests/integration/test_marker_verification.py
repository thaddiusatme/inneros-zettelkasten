...
class TestMarkerVerification:
    ...
    def test_extract_markers_from_file_without_marker(
        self, mock_test_file_without_marker
    ):
        """
        ...
        """
        markers = MarkerVerifier.extract_markers_from_file(
            mock_test_file_without_marker
        )
    ...
    def test_all_real_integration_tests_have_markers(self):
        ...
        files_with_markers, files_missing_markers = (
            MarkerVerifier.scan_integration_tests(tests_dir)
        )

        # RED Phase: This should pass after Iteration 3 migration added markers
        if files_missing_markers:
            missing_list = "\n".join(
                f"  - {f.relative_to(tests_dir)}" for f in files_missing_markers
            )
            pytest.fail(
        ...
        assert (
            len(files_with_markers) > 0
        ), "No integration test files found - verify test discovery"

        print(
            f"✅ Verified {len(files_with_markers)} integration test files have markers"
        )
...
class TestMarkerDocumentation:
    ...
    def test_marker_strategy_exists_in_workflow(self):
        ...
        assert (
            "marker" in content.lower() or "pytest.mark" in content.lower()
