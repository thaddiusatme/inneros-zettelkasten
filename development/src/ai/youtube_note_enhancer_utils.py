    ...
    def parse_structure(content: str) -> NoteStructure:
        ...
        try:
            ...
            if content.startswith("---"):
                yaml_section = (
                    content.split("---", 2)[1] if content.count("---") >= 2 else ""
                )
                # Check for template placeholders or unclosed brackets
                if (
                    "{{" in yaml_section
                    or "[" in yaml_section
                    and "]" not in yaml_section.split("[")[-1]
                ):
        ...
        if why_match:
            ...
            result.insertion_point = content.find(why_match.group(0)) + len(
                why_match.group(0)
            )
        else:
