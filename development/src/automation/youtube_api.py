...
def validate_note_file(note_path: Path) -> Optional[Tuple[Dict, str]]:
    ...
    if not note_path.exists():
        return (
            {"error": "not_found", "message": f"Note file does not exist: {note_path}"},
            404,
        )
    return None
...
def check_already_processed(
    frontmatter: Dict[str, Any], force: bool
...
def check_cooldown(
    note_path: Path, handler: "YouTubeFeatureHandler", force: bool
) -> Optional[Tuple[Dict, str]]:
    """
...
def create_youtube_blueprint(handler: "YouTubeFeatureHandler") -> Blueprint:
    ...
    def process_note():
        ...
        if not request.is_json:
            ...
            return (
                jsonify(
                    {"error": "invalid_request", "message": "Request must be JSON"}
                ),
                400,
            )
...
def _process_queue_worker(handler: "YouTubeFeatureHandler"):
    ...
    while True:
        try:
            ...
            # Store result
            job_results[job_id] = {"completed_at": time.time(), "result": result}
            ...
            if result.get("success"):
                ...
                logger.error(
                    f"Job {job_id} failed after {processing_time:.2f}s: "
