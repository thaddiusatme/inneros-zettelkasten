    ...
    def process_screenshot(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if ocr_results:
                ...
                self.logger.warning(
                    f"Screenshot processed but no OCR results: {file_path.name}"
                )
                return {"success": True, "ocr_results": {}, "screenshot_count": 0}

        except Exception as e:
            self.logger.error(
                f"OCR processing failed for {file_path.name}: {e}", exc_info=True
            )
...
class SmartLinkEngineIntegrator:
    ...
    def process_note_for_links(self, file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"Smart link analysis complete: {file_path.name} ({suggestions_count} suggestions)"
            )
            return {
                "success": True,
            ...
            self.logger.error(
                f"Smart link processing failed for {file_path.name}: {e}", exc_info=True
            )
            return {"success": False, "error": str(e), "suggestions_count": 0}
...
class ProcessingMetricsTracker:
    ...
    def record_processing_time(
        self, duration: float, threshold: Optional[float] = None
    ) -> None:
        """
    ...
    def export_metrics_json(self) -> str:
        ...
        export_data["total_events"] = (
            self.metrics["events_processed"] + self.metrics["events_failed"]
        )

        # Calculate success rate
        total = export_data["total_events"]
        export_data["success_rate"] = (
            (self.metrics["events_processed"] / total) if total > 0 else 0.0
        )
...
class ErrorHandlingStrategy:
    ...
    def handle_service_unavailable(
        logger: logging.Logger, service_name: str, filename: str
    ) -> None:
        """
    ...
    def handle_processing_error(
        logger: logging.Logger, error: Exception, filename: str
    ) -> None:
        """
