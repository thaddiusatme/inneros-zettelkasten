...
class VaultConfig:
    ...
    def __init__(self, config_path: Optional[Path] = None, base_dir: Optional[Path] = None):
        ...
        if config_path is None:
            ...
            candidates = [
                Path(__file__).parent.parent.parent / "vault_config.yaml",  # development/
                self.base_dir / "vault_config.yaml",  # project root
                self.base_dir / "development" / "vault_config.yaml",  # project_root/development/
            ]
    ...
    def inbox_dir(self) -> Path:
        ...
        inbox_name = self._config_data.get('vault', {}).get('directories', {}).get('inbox', 'Inbox')
        return self.vault_root / inbox_name
    
    @property
    def fleeting_dir(self) -> Path:
        """Get the Fleeting Notes directory path."""
        fleeting_name = self._config_data.get('vault', {}).get('directories', {}).get('fleeting', 'Fleeting Notes')
        return self.vault_root / fleeting_name
    
    @property
    def permanent_dir(self) -> Path:
        """Get the Permanent Notes directory path."""
        permanent_name = self._config_data.get('vault', {}).get('directories', {}).get('permanent', 'Permanent Notes')
        return self.vault_root / permanent_name
    
    @property
    def literature_dir(self) -> Path:
        """Get the Literature Notes directory path."""
        literature_name = self._config_data.get('vault', {}).get('directories', {}).get('literature', 'Literature Notes')
        return self.vault_root / literature_name
    
    @property
    def archive_dir(self) -> Path:
        """Get the Archive directory path."""
        archive_name = self._config_data.get('vault', {}).get('directories', {}).get('archive', 'Archive')
        return self.vault_root / archive_name
    
    @property
    def reports_dir(self) -> Path:
        """Get the Reports directory path."""
        reports_name = self._config_data.get('vault', {}).get('directories', {}).get('reports', 'Reports')
        return self.vault_root / reports_name
    
    @property
    def media_dir(self) -> Path:
        """Get the Media directory path."""
        media_name = self._config_data.get('vault', {}).get('directories', {}).get('media', 'Media')
    ...
    def get_directory(self, dir_name: str) -> Path:
        ...
        """
        dir_relative = self._config_data.get('vault', {}).get('directories', {}).get(dir_name)
