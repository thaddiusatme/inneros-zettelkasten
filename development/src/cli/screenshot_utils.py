...
class ScreenshotOCRProcessor:
    ...
    def process_screenshot(self, image_path: Path) -> Optional[VisionAnalysisResult]:
        ...
        """
        try:
            if not image_path.exists() or image_path.suffix.lower() not in [
                ".jpg",
                ".jpeg",
                ".png",
            ]:
            ...
            if result:
                logger.info(
                    f"OCR processed {image_path.name}: {result.confidence_score:.2f} confidence"
                )
                return result
            else:
    ...
    def process_batch(
        ...
        logger.info(
            f"Batch OCR processing completed: {len(results)}/{len(image_paths)} successful"
        )
        return results
...
class DailyNoteGenerator:
    ...
    def generate_daily_note(
        self,
        ocr_results: List[VisionAnalysisResult],
        ...
        try:
            ...
            yaml_content = self.generate_yaml_frontmatter(
                ocr_results, len(screenshot_paths)
            )
            ...
            content_sections.append(
                f"Processed {len(screenshot_paths)} screenshots from Samsung S23 via OneDrive.\n"
            )
            ...
            if ocr_results:
                ...
                for i, ocr_result in enumerate(ocr_results, 1):
                    ...
                    content_sections.append(
                        f"**Content Type**: {ocr_result.content_type}\n"
                    )
                    content_sections.append(
                        f"**Summary**: {ocr_result.content_summary}\n"
                    )

                    if ocr_result.extracted_text.strip():
                        content_sections.append(
                            f"**Extracted Text**: {ocr_result.extracted_text[:200]}...\n"
                        )
    ...
    def generate_yaml_frontmatter(
        self, ocr_results: List[VisionAnalysisResult], screenshot_count: int
    ) -> str:
        """
...
class SmartLinkIntegrator:
    ...
    def suggest_moc_connections(self, ocr_result: VisionAnalysisResult) -> List[str]:
        ...
        if any(
            keyword in ocr_result.content_summary.lower()
            or keyword in " ".join(ocr_result.main_topics).lower()
...
class SafeScreenshotManager:
    ...
    def __init__(self, knowledge_path: str):
        ...
        logger.info(
            f"Initialized SafeScreenshotManager with backup root: {self.backup_root}"
        )
    ...
    def rollback_from_backup(self, backup_path: str) -> bool:
        ...
        try:
            ...
            if inbox_backup.exists() and inbox_current.exists():
                ...
                logger.warning(
                    "Backup or current Inbox directory missing, skipping rollback"
                )
                return False
    ...
    def deduplicate_screenshots(self, screenshot_paths: List[str]) -> List[str]:
        ...
        logger.info(
            f"Deduplicated screenshots: {len(screenshot_paths)} → {len(deduplicated)}"
        )
        return deduplicated
