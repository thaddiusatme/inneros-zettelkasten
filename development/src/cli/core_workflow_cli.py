...
class CoreWorkflowCLI:
    ...
    def process_inbox(
        self, output_format: str = "normal", fast_mode: bool = False
    ) -> int:
        """
    ...
    def promote(
        ...
        try:
            ...
            # Promote note
            result = self.workflow_manager.promote_note(
                str(resolved_path), target_type.lower()
            )
    ...
    def auto_promote(
        ...
        try:
            ...
            if not (0.0 <= quality_threshold <= 1.0):
                print(
                    f"❌ Error: Quality threshold must be between 0.0 and 1.0 (got {quality_threshold})",
                    file=sys.stderr,
                )
                return 2
    ...
    def repair_metadata(
        self, execute: bool = False, output_format: str = "normal"
    ) -> int:
        """
...
Examples:
  ...
  python core_workflow_cli.py /path/to/vault report --export report.json
    ...
    process_parser = subparsers.add_parser(
        "process-inbox", help="Process all inbox notes"
    )
    ...
    report_parser.add_argument(
        "--export", metavar="PATH", help="Export report to JSON file"
    )
    ...
    repair_parser = subparsers.add_parser(
        "repair-metadata", help="Repair missing frontmatter metadata in Inbox notes"
    )
...
def main() -> int:
    ...
    try:
        if args.command == "status":
            ...
            return cli.report(output_format=args.format, export_path=args.export)
