...
class ScreenshotProcessor:
    ...
    def __init__(
        ...
        if self.onedrive_path:
            self.screenshot_detector = OneDriveScreenshotDetector(
        ...
        self.individual_orchestrator = IndividualProcessingOrchestrator(
            self.knowledge_path
        )
        ...
        logger.info(
            f"Initialized ScreenshotProcessor in {mode} mode for {knowledge_path}"
        )

    def scan_todays_screenshots(
        ...
        logger.info(
            f"Scan results: {len(all_screenshots)} total, {len(unprocessed)} unprocessed"
        )
        return unprocessed

    def scan_multi_device_screenshots(
        self, sort_by_timestamp: bool = False
    ) -> List[Path]:
        """
        ...
        for device_path in self.device_paths:
            ...
            screenshots = list(device_path.rglob("*.jpg")) + list(
                device_path.rglob("*.png")
            )
            ...
            for screenshot in screenshots:
                ...
                if device_type != DeviceType.UNKNOWN:
                    ...
                    logger.debug(
                        f"Found {device_type.value} screenshot: {screenshot.name}"
                    )
        ...
        logger.info(
            f"Found {len(all_screenshots)} screenshots across {len(self.device_paths)} device paths"
        )
    ...
    def process_multi_device_batch(self, limit: Optional[int] = None) -> Dict[str, Any]:
        ...
        ocr_results = self.ocr_processor.process_batch(
            screenshots, progress_callback=progress_callback
        )
        ...
        individual_note_paths = self._generate_individual_notes(
            screenshots, enriched_ocr_results
        )
    ...
    def _generate_individual_notes(
        self, screenshots: List[Path], ocr_results: Dict[str, Any]
    ) -> List[str]:
        """
        ...
        for i, screenshot in enumerate(screenshots, 1):
            ...
            if ocr_result:
                try:
                    ...
                    centralized_path = self.image_manager.save_to_attachments(
                        screenshot
                    )
                    logger.info(
                        f"Centralized screenshot: {screenshot.name} → {centralized_path}"
                    )

                    # Generate individual note using centralized path
                    note_path = self.individual_orchestrator.process_single_screenshot(
                        screenshot=centralized_path, ocr_result=ocr_result
                    )
                    ...
                    try:
                        screenshot.unlink()
                        logger.info(
                            f"Cleaned up original OneDrive screenshot: {screenshot}"
                        )
                    except Exception as cleanup_error:
                        logger.warning(
                            f"Could not delete original screenshot {screenshot}: {cleanup_error}"
                        )

                    success_count += 1
                    print(
                        f"   [{i}/{len(screenshots)}] ✅ Created: {Path(note_path).name}"
                    )
                    ...
                    failure_count += 1
                    logger.error(
                        f"Failed to create note for {screenshot.name}: {e}",
                        exc_info=True,
                    )
                ...
                print(
                    f"   [{i}/{len(screenshots)}] ⚠️  Skipped: {screenshot.name} (no OCR result)"
                )
    ...
    def process_batch(
        ...
        logger.info(
            f"Tracking stats: {tracking_stats['new_screenshots']} new, "
        ...
        if force:
            print("   ⚡ Force mode: Reprocessing all")
    ...
    def validate_file_accessibility(self, screenshots: List[Path]) -> Dict[str, Any]:
        ...
        for screenshot in screenshots:
            try:
                ...
                permission_errors.append(
                    {"file": str(screenshot), "error": f"Permission denied: {e}"}
                )
            except Exception as e:
                permission_errors.append(
                    {"file": str(screenshot), "error": f"Access error: {e}"}
                )
    ...
    def process_screenshots_with_ocr(
        ...
        for screenshot in screenshots:
            try:
                ...
                error_result.fallback_metadata = {
                    "filename": screenshot.name,
                    "size": screenshot.stat().st_size,
                }
                error_result.user_guidance = (
                    "OCR processing failed, check file format and accessibility"
                )
    ...
    def process_with_quality_assessment(
        self, screenshots: List[Path]
    ) -> Dict[str, Any]:
        """
    ...
    def generate_daily_note_with_images(
        self, ocr_results: Dict[str, VisionAnalysisResult], screenshots: List[Path]
    ) -> Path:
        """
...
"""
        ...
        for i, screenshot in enumerate(screenshots, 1):
            ...
            timestamp = (
                screenshot.name.split("_")[2] if "_" in screenshot.name else "unknown"
            )
            app_name = (
                screenshot.name.split("_")[3].replace(".jpg", "")
...
"""
    ...
    def organize_ocr_text_by_timestamp(self, screenshots: List[Path]) -> Dict[str, Any]:
        ...
        for screenshot in sorted(screenshots):  # Sort for chronological order
            ...
            if "_" in screenshot.name:
                ...
                if len(parts) >= 3:
                    ...
                    app_name = (
                        parts[3].replace(".jpg", "") if len(parts) > 3 else "unknown"
                    )
                else:
                    timestamp = "unknown"
                    app_name = "unknown"
        ...
        return {
            ...
            "average_confidence": (
                total_confidence / len(screenshots) if screenshots else 0.0
            ),
        }

    def prepare_for_smart_link_integration(
        self, screenshots: List[Path]
    ) -> Dict[str, Any]:
        """
    ...
    def process_batch_with_performance_tracking(
        self, screenshots: List[Path]
    ) -> Dict[str, Any]:
        """
    ...
    def process_with_progress_reporting(
        self, screenshots: List[Path], progress_callback=None
    ) -> Dict[str, Any]:
        """
        ...
        for i, screenshot in enumerate(screenshots):
            ...
            if i > 0:
                ...
                eta = (
                    total_screenshots * 2
                )  # Initial estimate of 2 seconds per screenshot
    ...
    def process_with_partial_failure_handling(
        self, screenshots: List[Path]
    ) -> Dict[str, Any]:
        """
        ...
        error_summary = {
            ...
            "success_rate": (
                len(successful_items) / len(screenshots) if screenshots else 0.0
            ),
        }
    ...
    def generate_individual_capture_notes(
        self, screenshots: List[Path], ocr_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for screenshot in screenshots:
            try:
                ...
                note_content = self.generate_template_based_note_content(
    ...
    def generate_contextual_filename(
        self, screenshot_path: Path, ocr_result: Any, timestamp: str
    ) -> str:
        """
        ...
        return self.filename_generator.generate_contextual_filename(
            screenshot_path, ocr_result, timestamp
        )

    def analyze_screenshot_with_rich_context(
        ...
        """
        # Delegate to extracted utility class
        return self.context_analyzer.analyze_screenshot_with_rich_context(
            screenshot_path
        )

    def generate_template_based_note_content(
        self, screenshot_path: Path, rich_context: Dict[str, Any], filename: str
    ) -> str:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.template_renderer.generate_template_based_note_content(
            screenshot_path, rich_context, filename
        )

    def extract_intelligent_description(
        self, ocr_text: str, content_summary: str
    ) -> str:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.filename_generator.extract_intelligent_description(
            ocr_text, content_summary
        )

    def generate_fallback_description(
        self, screenshot_path: Path, strategy: str
    ) -> str:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.filename_generator.generate_fallback_description(
            screenshot_path, strategy
        )

    def suggest_smart_links_for_individual_note(
        self, note_path: Path
    ) -> List[Dict[str, str]]:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.individual_link_integrator.suggest_smart_links_for_individual_note(
            note_path
        )

    def process_screenshots_individually_optimized(
        self, screenshots: List[Path]
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.individual_orchestrator.process_screenshots_individually_optimized(
            screenshots
        )

    def process_with_individual_progress_reporting(
        self, screenshots: List[Path], progress_callback=None
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.individual_orchestrator.process_with_individual_progress_reporting(
            screenshots, progress_callback
        )

    def process_individual_with_error_recovery(
        self, screenshots: List[Path]
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Delegate to extracted utility class
        return self.individual_orchestrator.process_individual_with_error_recovery(
            screenshots
        )
