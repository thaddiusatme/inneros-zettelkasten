...
def handle_suggest_links_command(args):
    ...
    try:
        ...
        ai_connections = AIConnections(
            similarity_threshold=args.min_quality, max_suggestions=args.max_results
        )
        ...
        # Filter by quality
        filtered_suggestions = filter_suggestions_by_quality(
            suggestions, args.min_quality
        )
        ...
        if args.interactive:
            ...
            results = orchestrator.execute_enhanced_interactive_workflow(
                filtered_suggestions, args.target, dry_run=args.dry_run
            )
            ...
            if not args.dry_run and "actions" in results:
                ...
                if preview_suggestions:
                    ...
                    for suggestion in preview_suggestions:
                        preview_result = link_inserter.preview_changes(
                            args.target, [suggestion]
                        )
                ...
                if accepted_suggestions:
                    ...
                    for suggestion in accepted_suggestions:
                        print(
                            f"   • {suggestion.suggested_link_text} → {suggestion.suggested_location}"
                        )

                    # Confirm insertion unless in non-interactive mode
                    print(f"\n📝 Target note: {args.target}")
                    confirmation = (
                        input("🤔 Proceed with link insertion? [Y/n]: ").lower().strip()
                    )

                    if confirmation in ["", "y", "yes"]:
                        try:
                            insertion_result = (
                                link_inserter.insert_suggestions_into_note(
                                    args.target,
                                    accepted_suggestions,
                            ...
                            if insertion_result.success:
                                print(
                                    f"✅ Successfully inserted {insertion_result.insertions_made} links"
                                )
                                if insertion_result.duplicates_skipped > 0:
                                    print(
                                        f"⚠️  Skipped {insertion_result.duplicates_skipped} duplicate links"
                                    )
                                if insertion_result.backup_path:
                                    print(
                                        f"💾 Backup created: {Path(insertion_result.backup_path).name}"
                                    )
                                ...
                                if insertion_result.insertions_made > 0:
                                    try:
                                        ...
                                        print(
                                            "↩️  Undo available this session: re-run with --undo"
                                        )
                                    except Exception as e:
                                        print(
                                            f"⚠️  Failed to record undo operation: {e}"
                                        )
                            else:
                                print(
                                    f"❌ Link insertion failed: {insertion_result.error_message}"
                                )

                        except Exception as e:
                            print(
                                f"❌ Unexpected error during link insertion: {str(e)}"
                            )
            ...
            reporter.start_batch(
                len(filtered_suggestions), "Link Suggestion Processing"
            )
...
def create_parser():
    ...
    similar_parser = subparsers.add_parser(
        "similar", help="Find notes similar to a target note"
    )
    ...
    suggest_parser = subparsers.add_parser(
        "suggest-links", help="Generate smart link suggestions with quality assessment"
    )
    ...
    if args.command == "suggest-links":
        ...
        if getattr(args, "undo", False):
            if _UNDO_MANAGER.can_undo():
                ...
                confirm = (
                    input(
                        "🤔 Proceed with undo (restore from backup if available)? [Y/n]: "
                    )
                    .lower()
                    .strip()
                )
    ...
    if args.command == "similar":
        ...
        try:
            ...
            total_connections = sum(
                len(connections) for connections in connection_map.values()
            )
            connected_notes = sum(
                1 for connections in connection_map.values() if connections
            )

            print("✅ Connection map built successfully!")
            print("-" * 60)
            print("📊 Statistics:")
            ...
            print(
                f"   Average connections per note: {total_connections / len(corpus):.1f}"
            )
            print()

            # Show top connected notes
            sorted_notes = sorted(
                connection_map.items(), key=lambda x: len(x[1]), reverse=True
            )
